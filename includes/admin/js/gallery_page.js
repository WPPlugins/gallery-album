wpda_gallery_tree={
	...
	create_content:function(){	
		...
		jQuery('.wpda_gall_tree_content_parent').append('<div class="wpda_gall_load_trash1"></div><div id="wpda_gall_tree_content" class="wpda_gall_tree_content"></div>');
		wpda_gall_tree_content = document.getElementById("wpda_gall_tree_content");					
	},
	//create add gallery item
	create_add_gallery:function(){		
		jQuery( ".wpda_gall_tree_content" ).eq(0).append( '<div title="add new gallery" class="wpda_gall_create_gallery"><div class="wpda_gall_tree_horizone_line"></div><span>Create Gallery</span></div>' );
	},
	// insert gallery
	create_gallery_into_tree:function(index){
		...
		jQuery( ".wpda_gall_tree_content" ).eq(0).append('<div class="wpda_gall_tree_open_close_gallery"> <span class="wpda_gall_tree_open_close_gallery_button" title="Expand/Collapse">+</span> <div class="wpda_gall_tree_open_close_gallery_right_span" data-gallery_index="'+index+'"></div><div class="wpda_gall_tree_open_close_gallery_horizon_line"></div><span class="wpda_gall_tree_galery_name" style="white-space:nowrap" title="edit Gallery">' + self.tree_info[index]['name'] + '</span></div>');
	...
	create_add_album:function(index){		
		...
		jQuery( ".wpda_gall_tree_gallery_content" ).eq(index).append('<div class="wpda_gall_tree_album_vertical_line"><div class="wpda_gall_tree_create_album" title="Create album"><div class="wpda_gall_tree_album_horizon_line"></div><span> Create album </span></div><span class="wpda_gall_tree_album_last"></span></div>');
	},	
	//insert albums
	...
	restore_iems:function(){
		...
		if(jQuery('.wpda_gall_standart_uploaded_image_status').length>0){
			var image_gallery_index = localStorage.getItem('wpda_gall_gallery_index');
			var image_album_index = localStorage.getItem('wpda_gall_album_index');
			jQuery('.wpda_gall_tree_gallery_content').eq(image_gallery_index).find('.wpda_gall_tree_album_content').eq(image_album_index).find('.wpda_gall_tree_create_img').trigger('click');
		}
	...
	conect_add_galery_to_form:function(){	
		...
		jQuery('.wpda_gall_create_gallery').click(function(){	
	...
	create_gallery_name_form:function(element){
		...
		// for security
		wp_nonce_securyty();
		jQuery('.wpda_gall_tree_gallery_form_name_conteiner').click(function(){event.stopPropagation();})
		jQuery('.wpda_gall_tree_gallery_form_cencel_button').click(function(){jQuery(this).parent().remove();event.stopPropagation();})

		jQuery(element).find("button").eq(0).click(function(event){
			event.preventDefault();
			var form_create_gallery_name=jQuery(element).find("input").attr("value");			
			var gallery_names = [];			
			for(x in wpda_gallery_tree.tree_info){
				gallery_names[x] = wpda_gallery_tree.tree_info[x].name;								
			}						
			if(form_create_gallery_name != ''&& !gallery_names.includes(form_create_gallery_name)){   
				...
				jQuery.ajax({
					type:'GET',
					url: wpda_gall_admin_ajax_url,
					data:{action:'wpda_gall_form_create_gallery', drag_g:form_create_gallery_name},
					timeout: 200, 
					...
					error: function(x, t, m) {					
						if(t==="timeout") {
							alert('got timeout; Tray again');jQuery('.wpda_gall_load_trash').removeClass('wpda_gall_display_inline');
						} else {
							alert(t);
						}					
					}					
	...
	conect_add_album_to_form:function(){	
		...
		jQuery('.wpda_gall_tree_create_album').click(function(){
			//remove other opened forms
			self.tree_remove_inline_opened_boxes();
			// create album				
	...
	create_album_add_name_form:function(element){
		...
		jQuery('.wpda_gall_tree_album_form_name_conteiner').click(function(){event.stopPropagation();})
		jQuery('.wpda_gall_tree_album_form_cencel_button').click(function(){jQuery(this).parent().remove();event.stopPropagation();})
		...
		jQuery(element).find("button").eq(0).click(function(event){
			event.preventDefault();
			var form_create_album_name=jQuery(element).find("input").attr("value");
			var albom_names = [];			
			...
			if(form_create_album_name != ''&& !albom_names.includes(form_create_album_name)){
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');						
				jQuery.ajax({
	...
	conect_edit_gallery_to_tree:function(){	
		...
		jQuery('.wpda_gall_tree_galery_name').click(function(){
	...
	create_edit_gallery_form_to_tree:function(element){
		...
		jQuery('.wpda_gall_edit_gallery_name_content').click(function(){event.stopPropagation();})
		jQuery('.wpda_gall_tree_gallery_edit_form_cencel_button').click(function(){self.remove_edit_gallery_form();return false;})
		...
		jQuery(element).next().children().eq(0).children().eq(1).click(function(event){
			event.preventDefault();
			var form_create_gallery_name=jQuery("[name='wpda_gall_tree_gallery_form_name_input']").attr("value");
			var gallery_names = [];			
			for(x in wpda_gallery_tree.tree_info){
				gallery_names[x] = wpda_gallery_tree.tree_info[x].name;								
			}						
			if(form_create_gallery_name != ''&& !gallery_names.includes(form_create_gallery_name)){ 		
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');
				var form_gallery_index_name=jQuery('.wpda_gall_edit_gallery_name_content form .wpda_gall_tree_gallery_form_name_input').attr("value");						
				jQuery.ajax({
		...
		jQuery(element).next().children().eq(0).children().eq(2).click(function(){
			event.preventDefault();
	...
	conect_edit_album_to_tree:function(){	
		...
		jQuery('.wpda_gall_tree_album_name').click(function(){
			//remove other appened forms
			self.tree_remove_inline_opened_boxes();
			// create album				
	...
	create_edit_album_form_to_tree:function(element){
		...
		jQuery('.wpda_gall_edit_album_name_content').click(function(){event.stopPropagation();})
		jQuery('.wpda_gall_tree_album_edit_form_cencel_button').click(function(){self.remove_edit_album_form();return false;})
		...
		jQuery(element).next().children().eq(0).children().eq(1).click(function(event){
			event.preventDefault();		
			var form_create_album_name=jQuery("[name='wpda_gall_tree_album_form_name_input']").attr("value");
			var albom_names = [];			
			for(x in wpda_gallery_tree.tree_info[form_gallery_index]){
				albom_names[x] = wpda_gallery_tree.tree_info[form_gallery_index][x].name;								
			}
			if(form_create_album_name != ''&& !albom_names.includes(form_create_album_name)){			
				jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');
				var form_album_index_name=jQuery('.wpda_gall_edit_album_name_content form .wpda_gall_tree_album_form_name_input').attr("value");						
				jQuery.ajax({
	...
	conect_edit_image_ayax:function(){		
		jQuery("[name='edit_image']").click(function(){
			...
			var form_img_index_description=jQuery("[name='wpda_gall_edit_image_description']").attr('value');			
			var form_img_index_name=jQuery("[name='wpda_gall_edit_image_name']").attr('value');
			jQuery.ajax({
	...
	dragEnter_rows:function(event) {
		if ( event.target.nodeName == "TD" && wpda_gallery_tree.drag_target == 'wpda_gall_drop_rows' ){ 
				...
				//jQuery(event.target).closest("tr").css('background-color', '#d57fab');
				jQuery(event.target).closest("tr").find("td").css('padding-top', '30px');
...
/////////////////////////////////////////////////   end   /////////////////////////////////////////////////////		
	...
	dragEnter:function(event) {
		if ( wpda_gallery_tree.drag_target != 'wpda_gall_drop_rows'){
			if (event.target.className == 'wpda_gall_tree_edit_img' || event.target.className == 'wpda_gall_tree_open_close_album_right_span' || event.target.className ==  'wpda_gall_tree_open_close_gallery_right_span' || event.target.className ==  'wpda_gall_load_trash1') {    
				if(wpda_gallery_tree.drag_target !=	1 && event.target.className != "wpda_gall_tree_open_close_gallery_right_span"){
			...
			if (event.target.id == 'wpda_gall_edit_album_form' && wpda_gallery_tree.drag_target != 1 && wpda_gallery_tree.drag_target != "wpda_gall_tree_open_close_gallery_right_span"){
				event.target.style.backgroundColor = "#ddffff";
			}
		}
	},
	...
	drag:function(ev) {
		...
		if("wpda_gall_edited_image" == ev.currentTarget.getAttribute("class")){this.drag_obect = aaa;}else{this.drag_obect = ev.currentTarget;};
	...
	drop:function(ev) {
		...
		if(wpda_gallery_tree.drag_target == 'wpda_gall_tree_edit_img' || wpda_gallery_tree.drag_target == 'wpda_gall_tree_open_close_album_right_span' || wpda_gallery_tree.drag_target ==  'wpda_gall_tree_open_close_gallery_right_span'){		
			...
			this.drop_obect = ev.currentTarget;
			//this.drop_obect.style.backgroundColor='#00ff00';
			...
			switch(wpda_gallery_tree.drag_drop) {
				...
				case "wpda_gall_tree_open_close_album_right_span wpda_gall_tree_open_close_album_right_span":
					...
					jQuery.ajax({
						...
						self.drag_album_drop_album();
						if(Object.keys(wpda_gallery_tree.tree_info[localStorage.getItem('wpda_gall_gallery_index')][localStorage.getItem('wpda_gall_album_index')]).length < 2){
				...
				case "wpda_gall_tree_open_close_album_right_span wpda_gall_load_trash1":						
					if(self.confirm_delete_album()) {self.dragLeave(ev); wpda_gallery_tree.drag_target = 1; return false;}
					jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline'); 
					jQuery.ajax({
				...
				case "wpda_gall_tree_open_close_gallery_right_span wpda_gall_load_trash1":
					if(self.confirm_delete_gallery())  {self.dragLeave(ev); wpda_gallery_tree.drag_target = 1; return false;}				
					jQuery('.wpda_gall_load_trash').addClass('wpda_gall_display_inline');
					jQuery.ajax({
			...
			//jQuery(this.drop_obect).remove('wpda_gall_drop_clas');
			self.dragLeave(ev);
			wpda_gallery_tree.drag_target = 1;
		}	
	},

	//1.inversia erku toxere
	...
	drag_img_drop_album:function() {	
		...
		var drop_album_value = Object.values(drop_album);
	...
	drag_album_drop_album:function() {
...
var wpda_gallery_edit_upload={
		...
		jQuery('.wpda_gall_add_image_tab_links ul li').click(function(){			
			...
			switch(jQuery('.wpda_gall_add_image_tab_links ul li').index(this)) {
				...
				case 2:
					...
					localStorage.setItem('wpda_gall_gallery_active_tab_index',2);
					wpda_gallery_edit_upload.restart_tree();jQuery('.wpda_gall_add_image_tab_links ul li').removeClass('active');jQuery(this).addClass('active');
					break;
			}	
		})				
	},
	create_media_attachments:function(){ 
		...
		if(localStorage.getItem('wpda_gall_attachments_or_upload') == 0) {
			...
			for (var i = 0; i < self.album_images_count; i++) {
				jQuery('#from_media_library').prepend('<div class="thumbnails with_checkbox" style="background-image:url(\''+self.upload_images[(self.current_page_index - 1) * self.count_image_per_page + i]['thumb_url']+'\');"><input type="checkbox" class="wpda_gall_media_checkbox" name="medi_library_attachment[]" value="'+self.upload_images[(self.current_page_index - 1) * self.count_image_per_page + i]['id']+'"></div>');
			}			
		}
	...
	select_with_shift_media:function(){	
		...
		chkboxes.click(function(e) {
			...
			if(self.shift_key_preset || e.shiftKey) {
	...
	media_attachemnt_thumbnail_click:function(){
		var self=this;
		jQuery('#from_media_library .thumbnails.with_checkbox').click(function(e){	
			if(e.shiftKey){
		...
		jQuery('#from_media_library .thumbnails.with_checkbox input').click(function(){
	...
	create_edit_album_elements:function(elements,gallery_index,album_index){ 
		...
		jQuery('#wpda_gall_album_caunt').text((Object.keys(elements).length-1)+' items');
		...
		if(localStorage.getItem('wpda_gall_redact_album_column') == 0){
			...
			for (var i = 0; i < self.album_images_count; i++) {
				...
				jQuery('#wpda_gall_edit_album_form table').append(''+
				  '<tr class="wpda_gall_drop_rows" data-gallery_index="'+gallery_index+'" data-album_index="'+album_index+'" data-img_index="'+((self.current_page_index - 1) * self.count_image_per_page + i)+'" ondragenter="wpda_gallery_tree.dragEnter_rows(event)" ondragleave="wpda_gallery_tree.dragLeave_rows(event)" ondrop="wpda_gallery_tree.drop_rows(event)" ondragover="wpda_gallery_tree.allowDrop(event)" draggable="true" ondragstart="wpda_gallery_tree.drag_rows(event)">'+
					...
					'<td><div class="thumbnails with_checkbox with_edit" style="background-image:url(\''+wpda_gall_content_url_width+elements[(self.current_page_index - 1) * self.count_image_per_page + i]['url']+'\');"><span data-gallery_index="'+gallery_index+'" data-album_index="'+album_index+'" data-img_index="'+((self.current_page_index - 1) * self.count_image_per_page + i)+'"><span>✎</span></span></div></td>'+
			...
			jQuery('input[name="save_album_images_name_description"]').mousedown(function(){
				jQuery('#wpda_gall_edit_album_form table tr td').children(2).attr('checked',true);
			...
			jQuery('input[name="wpda_gall_album_thumbnail_all"]').click(function(){
				if(this.checked){
					...
					jQuery('#wpda_gall_edit_album_form table tr td').children(2).attr('checked',false);
				}
			});				
			///////////18-05-2017					
		...
		jQuery('#wpda_gall_edit_album_form>h2 span').html(wpda_gallery_tree.tree_info[gallery_index]['name']+"/"+elements['name']);//19-05-2017
	...
	album_thumbnail_click:function(){
		var self=this;
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit').click(function(e){	
			if(e.shiftKey){
				self.shift_key_preset=true;
			}else{
				self.shift_key_preset=false;
		...
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit input').click(function(){
			if(jQuery(this).is( ":checked" )){
				jQuery(this).parent().addClass('active')				
			}else{
				jQuery(this).parent().removeClass('active')
	...
	album_thumbnail_edit_click:function(){
		var self=this;
		//05-05-2017
		jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit span, #wpda_gall_edit_album_form table tr td span').click(function(e){
			var loc_self=this;
			event.stopPropagation();
	...
	update_information_for_edit_image:function(image_info){
		...
		jQuery('#edited_image').attr('src',wpda_gall_content_url+image_info['url'])
	...
	create_pageintion_html:function(){
		...
		self.pagination_buttons[11].onclick = function() { 
			...
			//localStorage.setItem('wpda_gall_current_page_index',self.current_page_index);//11-05-2017
			jQuery('#from_media_library .thumbnails.with_checkbox').remove();
			jQuery('#wpda_gall_edit_album_form .thumbnails.with_checkbox.with_edit').remove();
...
jQuery(document).ready(function(e) {
	...
	wpda_gallery_tree.conect_edit_image_ayax();  //05-04-2017
	/////////11-05-2017
    //jQuery("#wpda_gall_edit_album_form table input,#wpda_gall_edit_album_form table textarea").mouseenter (function(){jQuery(".wpda_gall_drop_rows").draggable({ disabled: true })});
    //jQuery("#wpda_gall_edit_album_form table input,#wpda_gall_edit_album_form table textarea").mouseleave(function(){jQuery(".wpda_gall_drop_rows").draggable("enable")}); 	
	...
	switch(localStorage.getItem('wpda_gall_gallery_active_tab_index')) {
		...
		case '4':
			//edit image tab
			jQuery('.wpda_gall_tree_gallery_content').eq(localStorage.getItem('wpda_gall_gallery_index')).find('.wpda_gall_tree_album_content').eq(localStorage.getItem('wpda_gall_album_index')).find('.wpda_gall_tree_edit_img').eq(localStorage.getItem('0')).trigger('click');
			break;
	}		
});
