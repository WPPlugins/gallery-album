...
class wpda_gall_post_page_popup{
        	...
        	<script type="text/javascript">
				function insert_poll() {					  
					if(jQuery('#wpdevart_forms_id').val()!='0'){
						var tagtext;
						tagtext = '<p>[wpdevart_gallery lost_ids="'+wpda_gall_post_page_popup.lost_elements.join(",")+'" gallery_id="' + jQuery('#select_gallery').val()+'"  album_id="' + jQuery('#select_album').val()+'"   theme_id="' + jQuery('#wpda_gall_theme').val()+'" order_type="' + jQuery('#wpda_gall_oreder_type').val()+'"]</p>';
	...
	private function generete_thmes(){
		global $wpdb;
		$themes=$wpdb->get_results($wpdb->prepare('SELECT `id`,`name` FROM ' . wpdevart_gallery_databese::$table_names['theme']),ARRAY_A);
	...
	private function create_albom_checkboxes($checkboxs){
		...
		foreach($checkboxs as $id => $name){			
			$checkboxes.='<div class="ch_button"><label><input type="checkbox" checked data-name="'.$name.'" class="alboms_checkbox" value="'.$id.'"><span>'.$name.'</span></label></div>';
		}
		return $checkboxes;
	}
