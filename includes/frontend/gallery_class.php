...
class wpdevart_gallery_theme{
	...
	public function load_image_info(){
		...
		//$nkarner = $wpdb->get_results( "SELECT DISTINCT image_w, image_h, url,image_name FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE gallery = '$gallery_id' And album = '$album_id' And url != '' ORDER BY order_id ASC LIMIT " . $obj->start . ", " . $obj->limit);
	...
	private function generete_theme_array($theme_id){
		...
		foreach($params_geted_admin as $param_heading_key=>$param_heading_value){
			foreach($param_heading_value['params'] as $key=>$value){
				if(!isset($this->theme_requared_params[$key])){
	...
	public function shortcode($atts){
		...
		$gallerianer = $wpdb->get_col( "SELECT DISTINCT gallery FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE album IS NULL ORDER BY id ASC;");			      
		$gallerianer_id = $wpdb->get_col( "SELECT id FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE album IS NULL ORDER BY id ASC ;");
		...
		for ($x = 0; $x < count($gallery_tema); $x++) { 
			$xx=$gallery_tema[$x];
			$albumner_id = $wpdb->get_col( "SELECT id FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE gallery = '$gallerianer[$xx]' And album IS NOT NULL AND image_name IS NULL ORDER BY id ASC ;" );
		...
		<script>
			...
			//pagination buttons bar inicializacume	
			(function (){
				var i;	
				Glr_" . self::$prefix . ".pagination_buttons_bar = document.getElementById('wpda_gall_css_" . self::$prefix . "_pagination_buttons_bar_clas');
				...
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[11]);
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[0]);
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[12]);

				for(i = 1; i < 11; i++) { 
					Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[i]);
				}
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[13]);
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[10]);
				Glr_" . self::$prefix . ".pagination_buttons_bar.appendChild(Glr_" . self::$prefix . ".gall_pagination_buttons[14]);
		...
		if(self::$prefix==1){			
			...
			for ($x = 0; $x < $gallery_count ; $x++) { 
				...
				for ($col = 0; $col < $y; $col++) {
					...
					//$nkarner = $wpdb->get_results( "SELECT DISTINCT image_w, image_h, url,image_name FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE gallery = '$gallerianer[$x]' And album = '$albumner[$col]' And url != '' ORDER BY order_id ASC LIMIT 4;" );
					...
					//$z = $wpdb->get_var( "SELECT COUNT(*) FROM ".wpdevart_gallery_databese::$table_names['gallery']." WHERE gallery = '$gallerianer[$x]' And album = '$albumner[$col]' And image_name IS NOT NULL" );	//***************
	...
	private function generete_css(){
		...
		$output_css="<style>
			...
			.wpda_gall_css_" . self::$prefix . "_image_kmmtc {
				...
				background-clip: padding-box;		/*************************************** avelacac 02-03-2017************************************/				
			}
			...
			/*      activ       */
			.wpda_gall_css_" . self::$prefix . "_album_kmmtc:active .wpda_gall_cs_rotate0 {
			...
			.wpda_gall_css_" . self::$prefix . "_album_kmmtc:active .wpda_gall_cs_rotate1 {
			...
			.wpda_gall_css_" . self::$prefix . "_album_kmmtc:active .wpda_gall_cs_rotate2 {
			...
			.wpda_gall_css_" . self::$prefix . "_album_kmmtc:active .wpda_gall_cs_rotate3 {
